...
def test(step, dataset_test, name, n_share, G, Cs, softmax_layer, softmax_mode, max_acc,
         open=False, mode="train", entropy=False, save_path="record/"):
    ...
    if acc_close_all > max_acc:
        ...
        print('save model!')
        save_model(G, Cs[0], Cs[1], "%s/pre/model/model_pre.pth" % (save_path))
        print("acc:%.4f,auc:%.4f" % (acc_close_all,100. * roc))
        print("save adapt_pred.txt!")
        gen_submission_files(('%s/pre/model/adapt_pred.txt' % (save_path)),dataset_test.dataset.imgs, prediction, anomaly_score)
    print("save adapt_pred.txt!")
    gen_submission_files('adapt_pred.txt',dataset_test.dataset.imgs, prediction, anomaly_score)
    return acc_close_all, roc, out_t, out_open, label_t
...
def test_pretrained(step, dataset_test, name, n_share, G,
    ...
    dataset_test_wanted = []
    for i in range(len(dataset_test.dataset.imgs)):
        dataset_test.dataset.imgs[i] = dataset_test.dataset.imgs[i][36:]#去掉绝对目录，改成从val_data开始的相对目录
